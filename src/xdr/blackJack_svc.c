...
GameStatus *startgame_1_svc(void *any, struct svc_req *rep){
	...
	strcat(gameStatus->msg, "**********Game starts**********");
...
Card* Init2Cards(){
	// 分配两张牌的内存
	Card* twoCards = (Card*)malloc(2 * sizeof(Card));
	if(twoCards == NULL){
		// 内存分配失败
...
void OneMoreCard(GameStatus *status, int id){
	...
	{
	case 1:{
		status->player.cards.cards_val = (Card *)realloc(status->player.cards.cards_val, (++status->player.cards.cards_len) * sizeof(Card));
		// 将新发的牌加入手牌
		status->player.cards.cards_val[status->player.cards.cards_len - 1] = CARDS[rand()%13];
		break;
	}
